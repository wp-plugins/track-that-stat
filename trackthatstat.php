...
<a style="float:none;" class="link" href="admin.php?page='.TRACK_PLUGIN_NAME.'&track_opt=skipoptin">Click Here Now To Access The Plugin Without Opting In</a>
		...
		?>
			...
			$(document).ready(function(){
				var redirect=$(".af-form-wrapper input[name='redirect']").val();

				var hidden_obj=$(".af-form-wrapper input[name='redirect']");
				$(".af-form-wrapper input[name='redirect']").remove();
				$(".af-form-wrapper").append('<input type="hidden" name="redirect66" id="'+$(hidden_obj).attr('id')+'" value="'+redirect+"&aweber_redirect=true&val=<?php echo($cookie_val);?>"+'" />');
				
				$(".af-form-wrapper").append('<input type="hidden" name="redirect" id="88'+$(hidden_obj).attr('id')+'" value="'+redirect+"&aweber_redirect=true&val=<?php echo($cookie_val);?>"+'" />');
				$(".af-form-wrapper").append('<input type="hidden" name="meta_redirect_onlist" value="'+redirect+"&aweber_redirect=true&val=<?php echo($cookie_val);?>"+'" />');
...
<div style="width:1000px;height:130px;">
		<h3 align="center"><a href="http://trackthatstat.org" target="_blank">TTS Blog</a>  | FAQ | <a href="http://trackthatstat.org/contact-us/" target="_blank">Support</a> </h3>
          
			
			  <p><span class="style1">Thank you for using our new plugin. If you have any questions please feel free to use the support button above. Also, check out <a href="http://trackthatstat.org/likes/headway.html" target="_blank"><strong>Headway Themes</strong></a>.</span><span class="style1"> It allows you to create completely unique design with an easy to use drag and drop interface. Don't use premade themes, create them your self. ;-) </span> </p>
			  <a target="_blank" href="http://trackthatstat.org/likes/headway.html"><img src="http://track.gohardmarketingl.netdna-cdn.com/wp-content/uploads/2012/02/headway.jpg" alt="Headway — The Drag & Drop Theme For WordPress" border="0"></a>
...
<!----------------------------------------------- header ------------------------------------------------------>
	...
	<div class="header">
        ...
        <div class="header_right">
        	<div class="menu_bg">
            	<ul class="menu">
                	<li><a <?php echo($_REQUEST['details'] == "referral" ? 'class="active"' : '');?> href="../../track-that-stat/admin.php?page=<?php echo(TRACK_PLUGIN_NAME);?>&amp;details=referral">Referrals</a></li>
                    <li><a <?php echo($_REQUEST['details'] == "keyword" ? 'class="active"' : '');?> href="../../track-that-stat/admin.php?page=<?php echo(TRACK_PLUGIN_NAME);?>&amp;details=keyword">Keywords</a></li>
                    <li><a <?php echo($_REQUEST['details'] == "page" ? 'class="active"' : '');?> href="../../track-that-stat/admin.php?page=<?php echo(TRACK_PLUGIN_NAME);?>&amp;details=page">Content Viewed</a></li>
                    <li><a <?php echo($_REQUEST['details'] == "visitor" ? 'class="active"' : '');?> href="../../track-that-stat/admin.php?page=<?php echo(TRACK_PLUGIN_NAME);?>&amp;details=visitor">Visitor Details</a></li>
                    <?php
						if(isset($_REQUEST['details'])) {
					?>
						<li><a href="../../track-that-stat/admin.php?page=<?php echo(TRACK_PLUGIN_NAME);?>">Main</a></li>
					<?php
						}
					?>
                </ul>
...
<?php
		...
		{	
			...
			if( empty($loaded)) {
				...
				$path = WP_PLUGIN_URL."/".TRACK_PLUGIN_NAME."/";
				$content .= "<script type='text/javascript' src='".$path."js/jquery.min.js'></script>";
				$content .= "<script type='text/javascript' src='".$path."js/trackthatstat.php'></script>";
			}
		...
		function _assert_db_structure() {
			...
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_trafficstats");	
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_referrer_stats");	
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_keyword_stats");	
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_visitors");	
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_online_status");	
			$wpdb->query("DROP TABLE IF EXISTS ".$wpdb->prefix."tts_settings");	


			//creating table for traffic within the site
			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_trafficstats (`stat_post_id` int NOT NULL default '0', `stat_date` datetime NOT NULL default '0000-00-00 00:00:00' , `stat_visitor_id` VARCHAR( 32 ) NULL , `visitor_id` VARCHAR( 32 ) NULL , referrer_id int(11) NULL default '0') ENGINE = InnoDB");			
			
			//creating table for referrals
			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_referrer_stats (`id` bigint(20) NOT NULL AUTO_INCREMENT, `referrer` varchar(60) NOT NULL, `create_time` datetime NOT NULL,`post_id` int NOT NULL default '0',`stat_visitor_id` varchar(32) NULL, `visitor_id` VARCHAR( 32 ) NULL, PRIMARY KEY (`id`), KEY `referrer` (`referrer`)) ENGINE=InnoDB");
			 
			//creating table for keywords
			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_keyword_stats (`id` bigint(20) NOT NULL AUTO_INCREMENT, `keyword` varchar(60) NOT NULL, `create_time` datetime NOT NULL,`post_id` int NOT NULL default '0',`stat_visitor_id` varchar(32) NULL, `visitor_id` VARCHAR( 32 ) NULL, PRIMARY KEY (`id`), KEY `keyword` (`keyword`)) ENGINE=InnoDB");

			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_visitors(`id` bigint(20) NOT NULL AUTO_INCREMENT,`ip_address` varchar(20) not null default '',`country` varchar(50) default null,`browser` varchar(255) default null, `platform` varchar(255) default null, `page_url` text not null,`page_id` int(11) not null default '0',`time_visited` int(10) unsigned not null default '0',`time_last_visited` int(10) unsigned not null default '0',num_visits int(10) unsigned not null default '0',PRIMARY KEY (`id`)) ENGINE=InnoDB");


			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_online_status(`id` bigint(20) NOT NULL AUTO_INCREMENT, `ip_address` varchar(20) NOT NULL DEFAULT '',`last_active_time` int(10) unsigned NOT NULL DEFAULT '0',PRIMARY KEY (`id`),KEY `ip_address` (`ip_address`)) ENGINE=InnoDB");


			$wpdb->query("CREATE TABLE IF NOT EXISTS ".$wpdb->prefix."tts_settings(`list_ip_address` text) ENGINE=InnoDB");
		...
		*/
		function getCountryName()
		{
			...
			if(empty($country_name) || $country_name=='' || !isset($_COOKIE['countryDetails']) || $country_name=='-' || strlen($country_name)<2) {
				$user_geo_info = file_get_contents("http://api.ipinfodb.com/v3/ip-city/?key=2e6f0d88a31a7eeabae533c82b139fc239bb97390fd057b46ee12fc9876fb334&ip=$ip&format=json");
...
function get_total_visitors()
{
	...
	$query	=	"select count(stat_visitor_id) as total_visitors
				 from ".$wpdb->prefix."tts_trafficstats";
...
function get_page_visitors_data()
{
	...
	$query	=	"select stat_post_id,count(stat_visitor_id) as unique_visitors,
				count(stat_visitor_id) as visitors from ".$wpdb->prefix."tts_trafficstats where stat_post_id!=0 group by stat_post_id order by visitors desc limit 6";
