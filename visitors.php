...
class Visitors{
	function visitors_online()
	{
		...
		function getList() {
		  $.post("<?php echo $path; ?>getList.php", function(list) {
...
<?php
	...
	function view_visitors_data($duration,$fromDate,$toDate)
	{
		...
		var date = new Date();
		  $.post("<?php echo $path; ?>getListDetailed.php?time="+date.getTime(), function(list) {
			$("#visitor_container").html(list);
		  });
		}
...
<div class="cont" align="center">
    ...
    <div class="select-r"> <img src="<?php echo($path); ?>images/vis.png" alt="" /></div>
          ...
          <tr>
            ...
            <td width="145" class="t-top-left-next align_left">IP Address</td>
            <td width="134" class="t-top-left-next align_left">Country</td>
  ...
  <?php
		...
		foreach($result as $res)
		{
			...
			if(strlen($res->platform)>15) $platform = substr_replace($res->platform,'...',15); else $platform = $res->platform;
...
?>
			  <tr>
				...
				<td class="t-mid-left-next<?php echo($css_class);?> align_left"><?php echo($res->country); ?></td>
...
?>
			  <tr>
			   <td class="t-bot-left<?php echo($css_class);?>">&nbsp;</td>
				...
				<td class="t-bot-left-next<?php echo($css_class);?> align_left"><?php echo($res->country); ?></td>
